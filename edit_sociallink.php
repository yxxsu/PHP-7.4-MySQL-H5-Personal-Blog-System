...
$id = intval($_GET['id']??0);
$row = $pdo->query("SELECT * FROM sociallinks WHERE id={$id}")->fetch();
if(!$row){
...
if($_POST){
    ...
    if(empty($name) || empty($url)){
        ...
        $stmt = $pdo->prepare("UPDATE sociallinks SET name=?,url=?,sort=?,is_show=? WHERE id=?");
        $stmt->execute([$name,$url,$sort,$is_show,$id]);
        redirect('sociallinks_list.php');
    }
}
?>
    ...
    <div class="card p-4 shadow">
        ...
        <form method="post">
            <div class="mb-3">
                ...
                <input type="text" name="name" class="form-control" value="<?=xss_echo($row['name'])?>" required>
            </div>
            <div class="mb-3">
                <label>链接地址</label>
                <input type="url" name="url" class="form-control" value="<?=xss_echo($row['url'])?>" required>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                <label class="me-3">
                    <input type="checkbox" name="is_show" value="1" <?php if($row['is_show']==1)echo 'checked';?>> 是否前台显示
                </label>
            </div>
