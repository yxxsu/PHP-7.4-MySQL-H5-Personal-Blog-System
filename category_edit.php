...
$id=intval($_GET['id']??0);
$row = $pdo->query("SELECT * FROM category WHERE id={$id}")->fetch();
if(!$row){
...
if($_POST){
    ...
    $stmt = $pdo->prepare("UPDATE category SET name=?,sort=?,is_show=? WHERE id=?");
    $stmt->execute([$name,$sort,$is_show,$id]);
    redirect('category_list.php');
}
?>
<!DOCTYPE html>
...
<div class="container mt-4">
    ...
    <div class="card p-4 shadow mt-3">
        <form method="post">
            ...
            <div class="mb-3">
                <label><input type="checkbox" name="is_show" value="1" <?=$row['is_show']?'checked':''?>> 前台显示</label>
            </div>
